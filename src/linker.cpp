...
gb_internal bool n64_prepare_build_from_context(void) {
	...
	request.lto_kind = build_context.lto_kind;
	request.reloc_mode = build_context.reloc_mode;
	request.no_crt = build_context.no_crt;
...
gb_internal i32 n64_package_rom_from_linker(LinkerData *linker, String const &output_filename) {
	...
	request.object_paths = linker->output_object_paths;
	array_init(&request.foreign_libraries, permanent_allocator(), 0, linker->foreign_libraries.count);
	for (Entity *entity : linker->foreign_libraries) {
		GB_ASSERT(entity->kind == Entity_LibraryName);
		N64ForeignLibrary library = {};
		library.name = entity->token.string;
		library.extra_linker_flags = entity->LibraryName.extra_linker_flags;
		library.paths = entity->LibraryName.paths;
		array_add(&request.foreign_libraries, library);
	}
	return n64_package_rom(request).exit_code;
}
...
gb_internal i32 linker_stage(LinkerData *gen) {
	...
	if (build_context.metrics.os == TargetOs_n64 && build_context.build_mode == BuildMode_Executable) {
		return n64_package_rom_from_linker(gen, output_filename);
	}
